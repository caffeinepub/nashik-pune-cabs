import { Building2, MapPin, Navigation, Plane, RotateCcw } from "lucide-react";
...
const quickLinks: QuickLink[] = [
  ...
  },
  {
    label: "Mumbai → Nashik",
    icon: <Building2 className="h-5 w-5" />,
    data: { pickup: "Nashik CBS", drop: "Mumbai Central", tripType: "one-way" },
  },
  {
    label: "Nashik Local",
    icon: <MapPin className="h-5 w-5" />,
    data: { pickup: "Nashik CBS", drop: "Nashik Road", tripType: "one-way" },
  },
  {
    label: "Nashik → Mumbai",
    icon: <Building2 className="h-5 w-5" />,
      ...
      <div className="container">
        ...
        <div className="flex flex-wrap gap-3">
          {quickLinks.map((link) => (
            ...
            >
              <span className="text-saffron group-hover:text-charcoal">
                {link.icon}
              </span>
              {link.label}
            </button>
