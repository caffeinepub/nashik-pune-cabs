...
import { Loader2, Search, MapPin, Car, Calendar, Clock, User, Phone as PhoneIcon, Package, Users, IndianRupee, Navigation } from 'lucide-react';
import { useFindBookingByPhone } from '../hooks/useFindBookingByPhone';
import { useGetBookingById } from '../hooks/useGetBookingById';
import { CarCategory, CarModel, type Booking } from '../backend';
...
function BookingCard({ referenceId, booking }: { referenceId: string; booking: Booking }) {
  ...
  return (
    <div className="mt-6 space-y-4 rounded-xl border-2 border-saffron/30 bg-saffron/5 p-5">
      ...
      <div className="rounded-lg bg-saffron/10 p-3 text-center">
        ...
        <p className="mt-0.5 text-xl font-bold tracking-wide text-saffron">{referenceId}</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="flex items-start gap-3 rounded-lg border bg-background p-3">
          ...
          <div>
            ...
            <p className="font-semibold">{formatTimestamp(booking.timestamp)}</p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 rounded-lg border bg-background p-3">
          ...
          <div>
            ...
            <p className="font-semibold">{Number(booking.seats)} seat{Number(booking.seats) !== 1 ? 's' : ''}</p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 rounded-lg border bg-background p-3">
          ...
          <div>
            ...
            <p className="font-semibold">
              {Number(booking.luggage.count)} bag{Number(booking.luggage.count) !== 1 ? 's' : ''}
        ...
        <div className="flex items-start gap-3 rounded-lg border bg-background p-3">
          ...
          <div>
            ...
            <p className="font-semibold text-saffron">
              {Number(booking.price) > 0 ? `₹${Number(booking.price).toLocaleString('en-IN')}` : 'To be confirmed'}
            </p>
          </div>
        </div>
      </div>
...
export default function BookingLookup() {
  ...
  const [foundByPhone, setFoundByPhone] = useState<[string, Booking] | null>(null);

  const { mutate: findByPhone, isPending: phoneSearching } = useFindBookingByPhone();
  const { data: bookingById, isLoading: idSearching } = useGetBookingById(searchedId);
        ...
        <div className="mx-auto max-w-2xl">
          <div className="mb-8 text-center">
            ...
            <p className="text-muted-foreground">
              Enter your phone number or booking reference ID to view your booking details
            </p>
          </div>
          ...
          <Card className="border-2 shadow-lg">
            <CardHeader className="bg-gradient-to-r from-saffron/10 to-saffron/5 pb-4">
              ...
              <CardDescription>Search by phone number or booking reference ID</CardDescription>
            </CardHeader>
            <CardContent className="pt-5">
              ...
              <div className="mb-5 flex rounded-lg border border-border bg-muted/40 p-1">
                <button
                  type="button"
                  onClick={() => { setMode('phone'); setNotFound(false); setFoundByPhone(null); }}
                ...
                <button
                  type="button"
                  onClick={() => { setMode('id'); setNotFound(false); setSearchedId(''); }}
                  className={`flex-1 rounded-md py-2 text-sm font-semibold transition-all ${
                    mode === 'id'
                      ? 'bg-saffron text-charcoal shadow'
                      : 'text-muted-foreground hover:text-foreground'
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="bg-saffron text-charcoal hover:bg-saffron/90"
                    >
                      {idSearching ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
                    </Button>
                  </div>
                </div>
              )}
              ...
              {(notFound || showIdNotFound) && (
                <Alert className="mt-4" variant="destructive">
                  <AlertDescription>
                    No booking found. Please check your {mode === 'phone' ? 'phone number' : 'booking reference ID'} and try again.
                  </AlertDescription>
                </Alert>
              )}
              ...
              {mode === 'phone' && foundByPhone && (
                <BookingCard referenceId={foundByPhone[0]} booking={foundByPhone[1]} />
              )}
