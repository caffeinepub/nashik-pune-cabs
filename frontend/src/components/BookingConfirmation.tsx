...
export default function BookingConfirmation({
  ...
  const categoryLabel = bookingData.carCategory
    ? (CAR_CATEGORY_LABELS[bookingData.carCategory as CarCategory] ?? bookingData.carCategory)
    : bookingData.carType;

  const modelLabel = bookingData.carModel
    ? (CAR_MODEL_LABELS[bookingData.carModel as CarModel] ?? bookingData.carModel)
    : '';
    ...
    <Card className="mx-auto max-w-3xl border-2 shadow-xl">
      ...
      <CardContent className="space-y-6 pt-6">
        ...
        <div className="space-y-4">
          ...
          {bookingData.luggage && bookingData.luggage.count > 0 && (
            <div className="flex items-start gap-3 rounded-lg border p-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-muted-foreground">Luggage</p>
                <p className="font-semibold">
                  {bookingData.luggage.count} bag{bookingData.luggage.count !== 1 ? 's' : ''}
                  {bookingData.luggage.details ? ` — ${bookingData.luggage.details}` : ''}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <Button asChild className="flex-1 bg-saffron text-charcoal hover:bg-saffron/90">
