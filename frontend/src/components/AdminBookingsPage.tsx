...
import { Loader2, LogOut, ShieldAlert, Lock, ShieldCheck, Copy, CheckCircle2 } from 'lucide-react';
...
export default function AdminBookingsPage() {
  ...
  const { principalText, isLoading: isLoadingPrincipal } = useCurrentPrincipalText();
          ...
          <div className="container flex h-16 items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Bookings Management</p>
              </div>
            </div>
          </div>
        </header>
          ...
          <Card className="mx-auto max-w-2xl">
            ...
            <CardContent className="space-y-6">
              <Alert>
                ...
                <AlertDescription>
                  By claiming admin access, your Internet Identity principal will become the system administrator. 
                  This action can only be performed once and cannot be undone without backend intervention.
                </AlertDescription>
              </Alert>

              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">Your Principal ID</label>
                <div className="flex items-center gap-2">
                  <div className="flex-1 rounded-md border border-border bg-muted px-3 py-2">
                    {isLoadingPrincipal ? (
                      <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />
                    ) : (
                      <code className="text-xs break-all text-foreground">{principalText}</code>
                    )}
                  </div>
                  <Button
                    variant="outline"
                ...
                <p className="text-xs text-muted-foreground">
                  This principal will have full administrative access to view all bookings
                </p>
              </div>
              ...
              {initializeAdminMutation.isError && (
                <Alert variant="destructive">
                  <ShieldAlert className="h-4 w-4" />
                  <AlertDescription>
                    Failed to claim admin access: {initializeAdminMutation.error?.message || 'Unknown error'}
                  </AlertDescription>
                </Alert>
              )}
              ...
              <div className="flex justify-center">
                <Button
                  ...
                  disabled={initializeAdminMutation.isPending || isLoadingPrincipal}
                  size="lg"
                  className="bg-saffron text-charcoal hover:bg-saffron/90"
                >
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="container py-16">
          <Alert variant="destructive" className="mx-auto max-w-md">
            ...
            <AlertDescription>
              You do not have permission to access this page. Only authorized administrators can view bookings.
            </AlertDescription>
          </Alert>
        </main>
      </div>
